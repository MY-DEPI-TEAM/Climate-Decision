...
/* ============================================================
   Egypt Weather Database  —  SQL Server (Local) syntax
   ============================================================
   ملحوظة: جدول weather_monthly_current_year (بيانات السنة الحالية)
   ملوش وجود هنا خالص — مسؤوليته بالكامل جوه
   src/database/current_year_loader.py (staging + final table + refresh)
   ============================================================ */
...
SELECT
    CAST(CONCAT(ISNULL(governorate,'NA'), '_', FORMAT(date, 'yyyy-MM-dd')) AS VARCHAR(150)) AS id,
    governorate,
    date,
    weather_code,
    condition,
...
ALTER TABLE merged_weather
    ALTER COLUMN id VARCHAR(150) NOT NULL;

ALTER TABLE merged_weather
    ADD CONSTRAINT PK_merged_weather PRIMARY KEY (id);
...
SELECT
    CAST(revision_timestamp AS DATETIME2) AS revision_timestamp,
    [user],
    comment,
    source
INTO wikipedia_revision_history
